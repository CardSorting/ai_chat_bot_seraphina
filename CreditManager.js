...
class CreditManager {
  constructor(config = {}) {
    ...
    this.RENDER_COST = config.RENDER_COST || 5; // Added cost for ImagineCommand
  ...
  async fetchUserCredits(userID) {
    ...
    try {
      ...
      // Check if there's no existing entry for the user
  ...
  // Added the deduction function for the ImagineCommand cost
  async handleImagineCostDeduction(userID) {
